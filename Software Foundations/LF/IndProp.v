...
Theorem ev_double : forall n,
  ev (double n).
Proof.
  intros. induction n. simpl. apply ev_0. simpl. apply ev_SS. apply IHn. Qed.
(** [] *)
...
(* Fixpoint ev_ind (n: nat) (e: Prop) :=
  match e with
  | ev_0 => 0
  | ev_SS n e => ev_ind (pred n) (ev (pred n))
  end. *)
...
Proof.
  intros. 
  inversion H. inversion H1. inversion H3. Qed.
(** [] *)
...
Proof.
  ...
  - intros. induction H.
    + apply ev'_0.
    + assert (Hssn: forall nt: nat, S (S nt) = nt + 2).
      {
        induction nt.
        reflexivity.
        rewrite IHnt. simpl. reflexivity.
      }
      rewrite Hssn.
...
Lemma ABC_BAC: forall (A B C: Prop), (A->B->C) <-> (B->A->C).
Proof.  
  intros. split. 
  - intros. apply H in H1. apply H1. apply H0.    
  - intros. apply H in H1. apply H1. apply H0.
Qed.
...
Theorem ev_ev__ev : forall n m,
  ev (n+m) -> ev n -> ev m.
Proof.
  intros. induction H0 as [|n' IHEn']. simpl in H. apply H.
  apply IHIHEn'. simpl in H. remember (n'+m) as n. inversion H. apply H1. 
Qed.
(** [] *)

Lemma lemmaSS: forall (n m : nat), S n + S m = S (S (n + m)).
Proof.
  intros. simpl. generalize m.
  induction n.
  - intros. apply PeanoNat.Nat.succ_inj_wd.
    simpl. reflexivity.
  - intros. simpl. apply PeanoNat.Nat.succ_inj_wd.
    apply IHn.
Qed.
...
Proof.
  intros. 
  assert (Hevnn: forall (nt: nat), ev (nt+nt)).
  {
    induction nt. simpl. apply ev_0.
    rewrite lemmaSS. apply ev_SS. apply IHnt.
  }
  assert (Hnn: ev(n+n)). {apply Hevnn. }
...
Proof.
  intros. transitivity n. apply H. apply H0. Qed.
...
Theorem n_le_m__Sn_le_Sm : forall n m,
  n <= m -> S n <= S m.
Proof.
  intros. induction H as [|m' IH IH2].
  - apply le_n.
  - transitivity (S m'). apply IH2. apply (le_S _ _ (le_n (S m'))).
Qed.
...
Theorem Sn_le_Sm__n_le_m : forall n m,
  S n <= S m -> n <= m.
Proof.
  intros. inversion H. apply (le_n m). 
  transitivity (S n). apply (le_S _ _ (le_n n)). apply H1.
Qed.
...
Proof.
  intros. induction m. 
  - right. induction n. apply (le_n 0). apply le_S. apply IHn.
  - destruct IHm.
    + inversion H. 
      -- left. apply le_S. apply le_n.
      -- left. apply le_S. apply le_S. apply H0.
    + inversion H.
      -- left. apply le_n.
...
Proof.
  intros.
  induction b.
  - Search (_ + 0 = _). rewrite add_0_r. apply le_n.
  - Search (_ + S _ = S(_ + _)). rewrite PeanoNat.Nat.add_succ_r. apply le_S. apply IHb.
Qed.
...
Proof.
  intros. induction m.
  - inversion H. destruct n1. destruct n2.
    split. simpl. apply le_n. simpl. apply le_n. discriminate. discriminate.
  - inversion H.
    + split. apply le_plus_l. Search (?a + ?b = ?b + ?a). rewrite PeanoNat.Nat.add_comm.
      apply le_plus_l.
    + apply IHm in H1. destruct H1.
      split. apply (le_S _ _ H1). apply (le_S _ _ H2).
Qed.

(** Hint: the next one may be easiest to prove by induction on [n]. *)

Theorem predS: forall (n m : nat), 0 < n -> n + m = (pred n) + (S m).
Proof.
  intros. rewrite <- PeanoNat.Nat.add_succ_comm. Search (S (Nat.pred _)).
  Search (S (Nat.pred _)). apply Lt.S_pred_pos in H. rewrite <- H. reflexivity.
Qed.

Theorem Sp_lg_0: forall (p: nat), 0 < S p.
Proof.
  intros. induction p. apply le_n. apply le_S. apply IHp.
Qed.

Theorem add_le_cases : forall n m p q,
  n + m <= p + q -> n <= p \/ m <= q.
Proof.
  intros. generalize dependent m. generalize dependent p. generalize dependent q. induction n.
  - left. apply O_le_n.
  - intros. rewrite PeanoNat.Nat.add_succ_comm in H.
    destruct p.
    + simpl in H. apply plus_le in H. destruct H. right. transitivity (S m).
      apply (le_S m m (le_n m)). apply H0.
    + assert (0 < S p). {apply Sp_lg_0. } 
      apply predS with (m:= q) in H0. rewrite H0 in H. simpl in H. 
      apply IHn in H. destruct H. 
      -- left. simpl. apply n_le_m__Sn_le_Sm. apply H.
...
Theorem plus_le_compat_l : forall n m p,
  ...
  p + n <= p + m.
Proof.
  intros. induction H as [|m' IH IHIH].
  - apply le_n.
  - transitivity (p + m'). apply IHIH. rewrite <- PeanoNat.Nat.add_succ_comm. simpl.
    apply (le_S (p+m') (p+m') (le_n (p+m'))).
Qed.
...
Proof.
  intros. 
  rewrite PeanoNat.Nat.add_comm. rewrite PeanoNat.Nat.add_comm with (n:=m) (m:=p). 
...
Theorem le_plus_trans : forall n m p,
  ...
  n <= m + p.
Proof.
  intros. transitivity m. apply H. apply le_plus_l. Qed.

Theorem n_lt_m__n_le_m : forall n m,
  n < m ->
  n <= m.
Proof.
  intros. transitivity (n+1). rewrite PeanoNat.Nat.add_1_r.
  apply (le_S n n (le_n n)). rewrite PeanoNat.Nat.add_1_r. apply H.
Qed.
...
Proof.
  ...
  - inversion H.
  - inversion H. 
    + split. apply n_le_m__Sn_le_Sm. apply le_plus_l. apply n_le_m__Sn_le_Sm. 
      rewrite PeanoNat.Nat.add_comm. apply le_plus_l.
    + split. transitivity ( S (n1 + n2) ). apply n_le_m__Sn_le_Sm. apply le_plus_l. 
      apply n_le_m__Sn_le_Sm. apply H1. unfold lt. apply n_le_m__Sn_le_Sm. 
      unfold lt in H. apply Sn_le_Sm__n_le_m in H. transitivity (n1 + n2).
      rewrite PeanoNat.Nat.add_comm. apply le_plus_l. apply H.
Qed.
...
Proof.
  ...
  - intros. destruct n.
    + simpl. reflexivity.
    + simpl. apply Sn_le_Sm__n_le_m in H. apply IHm in H. apply H.
Qed.  
...
Proof.
  intros. apply leb_correct. apply leb_complete in H. apply leb_complete in H0.
...
Theorem leb_iff : forall n m,
  n <=? m = true <-> n <= m.
Proof.
  intros. split. apply leb_complete. apply leb_correct. Qed.
(** [] *)
...
Proof.
  ...
  - intros. induction H.
    ...
    + rewrite lemmaSS in IHR. injection IHR. intros. apply H0.
    + rewrite PeanoNat.Nat.add_comm. apply IHR.
  - intros. generalize dependent o. induction m.
    + induction n.

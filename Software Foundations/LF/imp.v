...
Proof.
  ...
  repeat (try rewrite optimize_0plus_sound);
  try reflexivity; try rewrite IHb; try reflexivity;
  try (rewrite IHb1; try rewrite IHb2); try reflexivity.
Qed. 
...
Inductive bevalR: bexp -> bool -> Prop :=
  | EBTrue : BTrue ==>b true
  | EBFalse : BFalse ==>b false
  | EBEq (a1 a2: aexp) : forall (n1 n2 : nat), a1 ==> n1 -> a2 ==> n2 -> bevalR (BEq a1 a2) (n1 =? n2)
  | EBLe (a1 a2: aexp) : forall (n1 n2 : nat), a1 ==> n1 -> a2 ==> n2 ->(BLe a1 a2) ==>b (n1 <=? n2)
  | EBNot (b : bexp) : forall (bl : bool), (b ==>b bl) -> BNot b ==>b negb bl
  | EBAnd (b1 b2 : bexp) : forall (bl1 bl2 : bool), b1 ==>b bl1 -> b2 ==>b bl2 -> BAnd b1 b2 ==>b andb bl1 bl2 
where "e '==>b' b" := (bevalR e b) : type_scope
.

Lemma BNot_BNot_is_itself: forall (b: bexp), beval (BNot (BNot b)) = beval b.
Proof.
  intros. induction b; simpl; try reflexivity; try rewrite negb_involutive; try reflexivity.
Qed.
...
Proof.
  split. 
  + intros; induction H; subst; simpl; try rewrite aeval_iff_aevalR in H;  try rewrite H; 
...
Proof.
  repeat try eapply E_Seq; try eapply E_Asgn; simpl; 
  try reflexivity. 
Qed.
(** [] *)
...
Proof.
  ...
  unfold pup_to_n.
  eapply E_Seq. eapply E_Asgn. reflexivity. 
  try eapply E_WhileTrue. simpl. try reflexivity.
  eapply E_Seq. eapply E_Asgn. reflexivity.
  ...
  repeat rewrite mul_0_r. simpl. 
  rewrite t_update_eq. apply E_WhileFalse. simpl.
  reflexivity.
Qed.

(*   unfold pup_to_n.
  eapply E_Seq. eapply E_Asgn. auto.
  try eapply E_WhileTrue; simpl; try reflexivity.
  eapply E_Seq; eapply E_Asgn; auto.
  try eapply E_WhileTrue; simpl; try reflexivity.
  eapply E_Seq; eapply E_Asgn; auto. 

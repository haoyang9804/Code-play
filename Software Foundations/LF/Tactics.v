...
Proof.
  ...
  injection H. intros. rewrite -> H1. rewrite -> H2. reflexivity. Qed.
(** [] *)
...
(* This one shows the details of discriminate without any use
of this tactic *)
...
Proof.
  ...
  | [ ] => False
  | _ => True
  end) [ ]). rewrite <- H. reflexivity. Qed.

(* This one shows how to use discriminate to prove *)

Example discriminate_ex3_copy :
  forall (X : Type) (x y z : X) (l j : list X),
    x :: y :: l = [] ->
    x = z.
Proof.
...
Proof.
  ...
  - intros. destruct n.
    + discriminate.
    + simpl. apply IHl'. simpl in H. injection H as Hi. 
    apply Hi. Qed. 
(** [] *)
...
Proof.
  ...
  - intros. destruct l1.
    + destruct l2.
      -- simpl in H. destruct x. destruct (split l). discriminate.
      -- simpl in H. destruct x. destruct (split l). discriminate.
    + destruct l2.
      -- simpl in H. destruct x. destruct (split l). discriminate.
      -- simpl in H. destruct x. destruct (split l).
         injection H. intros. rewrite H1. rewrite H3.
         simpl. 
         assert (MHY: forall {X : Type} (A B : list X) (x : X),
         A = B -> x :: A = x :: B).
         {
            intros. rewrite H4. reflexivity.
         }
         apply MHY. apply IHl. rewrite H0. rewrite H2.
         reflexivity. Qed.

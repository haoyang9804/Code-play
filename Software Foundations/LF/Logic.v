...
Proof.
  intros.
  unfold not in H. apply H in H0. destruct H0. Qed.
(** [] *)
...
Theorem contrapositive : forall (P Q : Prop),
  (P -> Q) -> (~Q -> ~P).
Proof.
  intros. unfold not. intros. apply H in H1. apply H0 in H1. apply H1.
Qed.
(** [] *)
...
Theorem iff_trans : forall P Q R : Prop,
  (P <-> Q) -> (Q <-> R) -> (P <-> R).
Proof.
  intros. transitivity Q. apply H. apply H0. Qed.
(** [] *)
...
Proof.
  intros. unfold not. intros [x H']. apply H' in H. apply H. Qed.
(** [] *)
...
Proof.
  ...
  - induction l.
    + intros. unfold map in H. unfold In in H. exfalso. apply H.
    + intros. destruct H.
      -- exists x. split. apply H. simpl. left. reflexivity.
      -- apply IHl in H. destruct H. destruct H.
         exists x0. split. apply H. simpl. right. apply H0.
  - induction l.
    + intros. destruct H as [x [H1 H2]]. contradiction.
    + intros. simpl. destruct H as [x0 [H1 H2]]. simpl in H2. destruct H2.
      -- left. rewrite H. apply H1.
      -- right. rewrite <- H1. apply In_map. apply H.
Qed.

Lemma trylemma: forall (F : nat -> Prop) (x : nat), ((True -> (exists x, F x)) -> F x).
Proof.
...
Proof.
  ...
  - intros. unfold iff. split.
    ...
    + simpl. intros. destruct H. exfalso. apply H. apply H. 
  - intros. unfold iff. split.
...
Proof.
  ...
  - intros. induction l.
    + exfalso. simpl in H0. apply H0.
    + simpl in H. destruct H. simpl in H0. destruct H0.
      -- rewrite <- H0. apply H.
      -- apply IHl in H1. apply H1. apply H0.
Qed.
(** [] *)
...
Example lemma_application_ex :
  forall {n : nat} {ns : list nat},
    ...
    n = 0.
Proof.
  intros n ns H. Check (proj1 _ _ (In_map_iff _ _ _ _ _)).
  destruct (proj1 _ _ (In_map_iff _ _ _ _ _) H)
           as [m [Hm Hn]].
  rewrite mul_0_r in Hm. rewrite <- Hm. reflexivity.
Qed.

Lemma trylemma2: forall (A B C : Prop), A -> B -> (A -> B -> C) -> C.
Proof.
  intros. apply H1 in H as H'. apply H'. apply H0.
Qed.
...
Proof.
  ...
  intro l. unfold tr_rev. rewrite rev_append_app. 
  apply app_nil_r.
Qed.
(** [] *)
...
Proof.
  ...
  - rewrite even_S. destruct (even n).
    ...
    + destruct IHn. simpl. rewrite H. exists (S x). reflexivity. 
Qed.
...
Proof.
  intros. split.
  - intros. split. destruct b1. reflexivity. discriminate H. destruct b2. reflexivity. destruct b1. discriminate. discriminate.
...
Lemma HHH_test_destruct_inductive_prop : forall (P P0: Prop), False  -> P.
Proof.
  intros. destruct H.
  - apply H.
  - apply H.
...
Proof.
  unfold not. intros. unfold excluded_middle in H. destruct H with (P:=(P x)). apply H1.
  unfold not in H1. exfalso. apply H0. exists x. apply H1. Qed. 
(** [] *)

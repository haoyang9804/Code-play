...
Fixpoint nonzeros (l:natlist) : natlist :=
  ...
  | nil => nil
  | h :: t => 
    if h =? 0 then nonzeros(t) else h :: nonzeros(t)
  end.
...
Fixpoint oddmembers (l:natlist) : natlist :=
  ...
  | nil => nil
  | h :: t =>
    if odd(h) then h :: oddmembers(t) else oddmembers(t)
  end.
...
Fixpoint count (v : nat) (s : bag) : nat :=
  ...
  | nil => 0
  | h :: t =>
    if v =? h then 1 + (count v t) else count v t
  end.
...
Fixpoint subset (s1 : bag) (s2 : bag) : bool :=
  ...
  | nil => true
  | h :: t =>
    if count h s2 =? 0 then false else subset t (remove_one h s2)
  end.
...
Theorem rev_reverse_asso: forall a b c: natlist, a ++ b ++ c = (a ++ b) ++ c.
Proof.
  intros.
  induction a as [|n' a' IHa'].
  - simpl. reflexivity.
...
Proof.
  ...
  - simpl. rewrite -> IHl1'. 
  rewrite -> rev_reverse_asso. reflexivity.
  Qed.
...
Proof.
  intros.
  induction l1 as [|n l1' IHl1'].
  - simpl. rewrite -> rev_reverse_asso. reflexivity.
  - simpl. 
    rewrite -> rev_reverse_asso. rewrite -> rev_reverse_asso.
    reflexivity.
    Qed.
...
Proof.
  ...
  induction l as [|n l' IHl'].
  - simpl. reflexivity.
  - destruct (n=?n) eqn:Eqn.
    + simpl. rewrite -> Eqn. rewrite -> IHl'. 
      reflexivity.
    + rewrite -> eqb_refl in Eqn. 
      discriminate Eqn.
      Qed.
(** [] *)
...
(* haoyang *)
Lemma unequal_rev_unequal : 
forall x y : natlist, rev x <> rev y -> x <> y.
Proof.
  induction x as [|n x' IHx']. 
  - intros. destruct y as [|m y'].
    + contradiction.
    + discriminate.
  - intros. simpl in H. destruct y as [|m y'].
    + discriminate. (* + unfold not. intros. discriminate. (equals!!!) *)
    + simpl in H. unfold not. intros. injection H0. intros. rewrite H1 in H. rewrite H2 in H. contradiction.
  Qed. 

Lemma lemma_1 : forall n : nat, forall l : natlist, l ++ [n] <> [ ].
Proof.
  intros.
  unfold not. intros. 
  destruct l as [|m l'].
...
Lemma besideDiscriminate : forall n : nat, forall l : natlist, l ++ [n] <> [ ].
Proof.
  intros. destruct l as [|n' l'].
  - simpl. discriminate.
  - apply unequal_rev_unequal. simpl. apply lemma_1. 
  Qed.

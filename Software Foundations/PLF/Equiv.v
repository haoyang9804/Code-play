...
Proof.
  ...
  try eapply E_IfTrue; try eapply E_IfFalse; try eapply WhileFalse; try eapply WhileTrue; try eapply E_Seq; try eapply E_Skip; try eapply E_Asgn; simpl; try reflexivity; subst; try apply H.
Qed.
...
Proof.
  ...
  - intros. inversion H0; subst.
    ...
    rewrite H in H6. discriminate. apply H7.
...
Proof.
  unfold cequiv. intros. split.
  - intros. inversion H; subst. apply E_IfFalse; simpl.
    rewrite H5; simpl. reflexivity.
    apply H6. apply E_IfTrue. simpl. rewrite H5. reflexivity.
    apply H6.
  - intros. inversion H; subst. apply E_IfFalse. simpl in H5.
    rewrite negb_true_iff in H5; apply H5. apply H6.
    apply E_IfTrue. simpl in H5. rewrite negb_false_iff in H5; apply H5. apply H6.
Qed.
...
Proof.
  ...
  apply while_true_nonterm with (c:=c) (st:=st) (st':=st') (b:=b) in H; 
  split; intros.
  - apply H in H0. contradiction.
  - assert (H'': ~( st =[ while true do skip end ]=> st' )).
    {
      intros. eapply while_true_nonterm. unfold bequiv. intros. 
      simpl. reflexivity.
    }
    apply H'' in H0. contradiction.
Qed.
...
Proof.
  unfold cequiv. intros. split; intros; inversion H; subst.
  - inversion H2; subst. eapply E_Seq. apply H3. 
    apply E_Seq with (c1:=c2) (c2:=c3) (st:=st'1) (st':=st'0) (st'':=st'). 
    apply H7. apply H5.
  - inversion H5; subst.
    apply E_Seq with (c1:= <{c1; c2}> ) (c2:=c3) (st:=st) (st':=st'1) (st'':=st').
    apply E_Seq with (c1:= c1) (c2:=c2) (st:=st) (st':=st'0) (st'':=st'1).
...
Proof.
  ...
  - inversion H1; subst. 
    apply E_Seq with (c1:=c1') (c2:=c2') (st:=st) (st':=st'0) (st'':=st').
...
Proof.
  ...
  intros. split; intros; inversion H2; subst.
  eapply E_IfTrue. rewrite <- H. apply H8. rewrite <- H0.
  apply H9. 
  eapply E_IfFalse. rewrite <- H. apply H8. 
  rewrite <- H1. apply H9.
  eapply E_IfTrue. rewrite H. apply H8. rewrite H0.
  apply H9.
  eapply E_IfFalse. rewrite H. apply H8. 
...
Proof.
  ...
  try apply t_update_neq; try apply H;
...
Inductive ceval : com -> state -> state -> Prop :=
  ...
  | E_WhileTrue : forall st st' st'' b c,
      ...
      st' =[ while b do c end ]=> st'' ->
      st  =[ while b do c end ]=> st''
  | E_HAVOC : forall num x st,
      st =[havoc x]=> (x !-> num; st)
  (* | E_HAVOC : forall num x st st',
      st' = (x !-> num; st) ->
      st =[havoc x]=> st' *)
...
Proof.
  ...
  (* apply E_HAVOC with (num:=0) (x:=X) (st:=empty_st) (st':=(X !-> 0)).
  reflexivity. Qed. *)

Example havoc_example2 :
  empty_st =[ skip; havoc Z ]=> (Z !-> 42).
Proof.
  eapply E_Seq. try eapply E_Skip.  apply E_HAVOC.   

  (* eapply E_Seq; try eapply E_Skip.
  apply E_HAVOC with (num:=42) (x:=Z) (st:=empty_st) (st':=(Z !-> 42)).
  reflexivity. *)
Qed.
...
Proof. 
  ...
  - inversion H2; inversion H5; subst.
    apply E_Seq with (st' := (t_update st Y num0)).
    + apply E_HAVOC.
    + assert (J :  t_update (t_update st X num) Y num0 =  t_update (t_update st Y num0) X num).
      { apply t_update_permute.
        unfold not; intros; inversion H0. }
      rewrite -> J.
      apply E_HAVOC.
  - inversion H2; inversion H5; subst.
    apply E_Seq with (st' := (t_update st X num0)).
    + apply E_HAVOC.
    + assert (J :  t_update (t_update st X num0) Y num =  t_update (t_update st Y num) X num0).
      { apply t_update_permute.
        unfold not; intros; inversion H0. }
      rewrite <- J.
      apply E_HAVOC.

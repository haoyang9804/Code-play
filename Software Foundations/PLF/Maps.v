...
Lemma t_apply_empty : forall (A : Type) (x : string) (v : A),
  (_ !-> v) x = v.
Proof.
  intros. unfold "'_' '!->' v". reflexivity. Qed.
(** [] *)
...
Lemma t_update_eq : forall (A : Type) (m : total_map A) x v,
  (x !-> v ; m) x = v.
Proof.
  intros. unfold "x '!->' v ';' m". unfold eqb_string. 
  remember (string_dec x x). destruct s. reflexivity. exfalso. apply n. reflexivity.
Qed.
...
Proof.
  intros. unfold "x '!->' v ';' m". unfold eqb_string.
  remember (string_dec x1 x2). destruct s.
  assert (e_copy: x1 = x2). {apply e. } apply H in e_copy. 
...
Lemma t_update_shadow : forall (A : Type) (m : total_map A) x v1 v2,
  (x !-> v2 ; x !-> v1 ; m) = (x !-> v2 ; m).
Proof.
  intros. unfold "x '!->' v2 ';' m". unfold eqb_string.
  apply functional_extensionality. intros. remember (string_dec x x0).
...
Proof.
  unfold eqb_string. intros. remember (string_dec x y).
  destruct s. rewrite e. apply ReflectT. reflexivity.
...
Proof.
  intros. 
  remember (m x) as v. 
  unfold "x '!->' v ';' m". apply functional_extensionality.
  intros.
  destruct (eqb_stringP x x0).
  - rewrite e in Heqv. apply Heqv.
  - reflexivity.
Qed.
...
Theorem t_update_permute : forall (A : Type) (m : total_map A)
  ...
  (x2 !-> v2 ; x1 !-> v1 ; m).
Proof.
  intros. unfold "x1 '!->' v1 ';' m". apply functional_extensionality.
  intros. 
